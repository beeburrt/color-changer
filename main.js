...
const colors = [
  ...
  { color: "PaleVioletRed", hex: "#D87093" },
...
const mainDiv = bod.querySelector(".main-div");
const leftP = mainDiv.querySelector(".left-p");
const otherLeftP = mainDiv.querySelector(".other-left-p");
const btn = mainDiv.querySelector(".btn");
const clicksP = mainDiv.querySelector(".clicks-number");
const lastClrDiv = mainDiv.querySelector(".last-clr");
const nextClrDiv = mainDiv.querySelector(".next-clr");
lastClrDiv.textContent = "Last Color";
nextClrDiv.textContent = "Next Color";
...
function handleClick() {
  clickCount++;
  clicksP.textContent = `${clickCount}`;
  if (count < 141) {
    bod.style.backgroundColor = colors[count].hex;
    leftP.textContent = colors[count].color;
    otherLeftP.textContent = colors[count].hex;
    nextClrDiv.style.backgroundColor = colors[count + 1].hex;
  } else if (count === 141) {
    bod.style.backgroundColor = colors[count].hex;
    leftP.textContent = colors[count].color;
    otherLeftP.textContent = colors[count].hex;
    nextClrDiv.style.backgroundColor = "#F0F8FF";
    lastClrDiv.style.backgroundColor = "#FFFF00";
    count = -1;
  }
  if (count === 0 && clickCount === 1) {
    lastClrDiv.style.backgroundColor = "#FF69B4";
  } else if (count === 0) {
    lastClrDiv.style.backgroundColor = "#9ACD32";
  } else if (count < 142 && count > 0) {
    lastClrDiv.style.backgroundColor = colors[count - 1].hex;
  }
  count++;
}
